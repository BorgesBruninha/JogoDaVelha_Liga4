...

{
    ...
    int x, pos, b = 0, g = 0, contGanha1 = 0, contGanha2 = 0, contadorgeral = 0;   // var x = escoolha do x ou O ; pos = posição ;  b = é p linha ; g = p coluna.
    ...
    while(contadorgeral < 3 && contGanha1 != 2 && contGanha2 != 2) //Condição melhor de 3
    {
        ...
        char m[3][3]; //Preenche o vetor com espaçoes em branco
        for( int i = 0; i < 3; i++)
        {
            for(int j = 0; j < 3; j++)
            {
        ...
        do
        {
            ...
            {
                if(m[0][0] == j1) //Se primeira linha for completp pela variavel do jogador 1
                {
        ...
        while(ganhou != true && cont < 9); //Se ganhar ou o todas as posições forem preenchidas ele sai do do while

        for (int i = 2; i >= 0; i--) // Mostrar o jogo(tabuleiro) finalizado
        {
